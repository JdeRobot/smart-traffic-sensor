...
void rectify(Tpoint2D* src, Tpoint2D* dst, Mat& affineMat)
{
  Point2f srcPoints[4];
  srcPoints[0] = Point2f(src[0].x, src[0].y);
  srcPoints[1] = Point2f(src[1].x, src[1].y);
  srcPoints[2] = Point2f(src[2].x, src[2].y);
  srcPoints[3] = Point2f(src[3].x, src[3].y);

  Point2f rectifiedPoints[4];
  rectifiedPoints[0] = Point2f(dst[0].x, dst[0].y);
  rectifiedPoints[1] = Point2f(dst[1].x, dst[1].y);
  rectifiedPoints[2] = Point2f(dst[2].x, dst[2].y);
  rectifiedPoints[3] = Point2f(dst[3].x, dst[3].y);

  affineMat = getAffineTransform(srcPoints, rectifiedPoints);
}
